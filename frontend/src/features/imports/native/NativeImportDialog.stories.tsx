...
} satisfies Meta<typeof NativeImportDialog>;
...
export const FullImportFlow: Story = {
  play: async ({ canvas, userEvent, step }) => {
    await step("Verify file selection step is shown", async () => {
      await waitFor(() => {
        expect(
          canvas.getByText(/Drop Senticor Project export here/),
        ).toBeInTheDocument();
      });
    });

    await step("Upload a file", async () => {
      const input = canvas.getByTestId("native-file-input") as HTMLInputElement;
      const file = new File(["[{}]"], "items-export.json", {
...
export const ImportError: Story = {
  parameters: {
    msw: {
      handlers: [
        http.post("*/imports/native/from-file", () => {
