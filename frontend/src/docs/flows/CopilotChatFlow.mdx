...
<Meta title="Flows/Chat with Copilot" />

# Flow: Chat with Copilot

This document traces the user experience of chatting with Copilot, from opening the panel to accepting a suggestion that creates real items in the workspace.
...
graph LR
  ...
  S["<b>SUGGEST</b><br/>Copilot tool call"]
...
**Component**: `CopilotChatToggle` (in `AppHeader`) → `CopilotChatPanel`

**User story**: "I want to talk to Copilot about planning something. I click the sparkle icon in the header."

**Flow**:

1. User sees the `chat_bubble` icon button in `AppHeader` (between logo and hamburger menu)
2. Clicks the button → `CopilotChatPanel` slides in from the right edge
3. Panel is ~400px wide on desktop, full-screen on mobile
4. Empty state shows a welcome message: "Hallo! Ich bin Copilot. Wie kann ich helfen?"
...
**Components**: `ChatInput` → `ChatMessageList` → `UserMessageBubble`
...
2. Presses Enter → message submitted
3. `UserMessageBubble` appears right-aligned in `ChatMessageList`
4. `ChatInput` clears, ready for next message
5. `CopilotThinkingIndicator` appears (animated dots with `chat_bubble` avatar)
...
## Phase 3: Copilot Responds

**Components**: `CopilotMessageBubble` → `CopilotSuggestionCard`

**User story**: "Copilot understands what I need and shows me a suggestion."

**Flow**:

1. API returns a response with text + optional tool calls
2. `CopilotThinkingIndicator` is replaced by `CopilotMessageBubble` with text content
3. If tool calls are present, a `CopilotSuggestionCard` renders below the text
4. The suggestion card shows a preview of the proposed items

**Message types created**:

- `CopilotTextMessage` — the conversational text
- `CopilotSuggestionMessage` — the tool call rendered as a card (status: `"pending"`)

**Example**: Copilot responds to "birthday party planning" with:

```
CopilotMessageBubble: "Klingt nach einem Projekt! Hier ist mein Vorschlag:"

CopilotSuggestionCard:
  ┌──────────────────────────────────────┐
  │ ✦ Geburtstagsfeier planen            │
  │   Ergebnis: Erfolgreiche Feier       │
  │                                      │
...
**Component**: `CopilotSuggestionCard` (edit mode)

**User story**: "I like the suggestion but want to tweak a few things before accepting."

**Flow**:

1. User clicks the edit button (✎) on the suggestion card
2. Card enters edit mode (status: `"editing"`):
   ...
   - User can remove individual actions (✕ button)
   - Desired outcome is editable
3. User modifies the suggestion to their liking
4. Clicks Accept (✓) to confirm the edited version, or Cancel to return to pending
...
**Components**: `CopilotSuggestionCard` → `useCopilotActions` → `CopilotConfirmation`

### Accept path

**User story**: "This looks right — create it."

**Flow**:

1. User clicks [✓ Übernehmen]
2. `useCopilotActions` orchestrates item creation using existing mutation hooks:
   - `useCreateProject()` → creates the project
   - `useAddProjectAction()` → creates each action linked to the project
   - Item creation for references/documents as needed
3. Suggestion card status changes to `"accepted"`
4. Card transforms into a compact `CopilotConfirmation`:
...
- `Project` with `CaptureSource: { kind: "copilot", conversationId: "..." }`
- N × `ActionItem` with `bucket: "next"`, linked to project via `projectIds`
- Optional `ReferenceMaterial` for documents

### Dismiss path

**User story**: "Not what I wanted — skip this."

**Flow**:

1. User clicks [✗ Verwerfen]
...
5. Conversation continues — user can send another message

---

## Phase 6: Continue Conversation

**User story**: "I want to ask Copilot for more things in the same conversation."
...
<Mermaid chart={`
graph TD
  AH["AppHeader"] --> TT["CopilotChatToggle<br/><i>chat_bubble button</i>"]
  TT -->|"onClick"| TCP["CopilotChatPanel<br/><i>right side panel</i>"]

TCP --> CML["ChatMessageList<br/><i>scrollable</i>"]
CML --> UMB["UserMessageBubble"]
CML --> TMB["CopilotMessageBubble"]
CML --> TTI["CopilotThinkingIndicator"]
CML --> TSC["CopilotSuggestionCard"]
CML --> TC["CopilotConfirmation"]

TCP --> CI["ChatInput<br/><i>AutoGrowTextarea + send</i>"]
CI -->|"onSend"| UCS["useChatState<br/><i>local state: messages[]</i>"]
UCS --> UTA["useCopilotApi<br/><i>POST */chat/completions</i>"]
UCS -->|"on accept"| UTAC["useCopilotActions"]
...
graph LR
  ...
  TA["<b>useCopilotApi</b><br/>POST */chat/completions"]
  TAC["<b>useCopilotActions</b><br/>executeToolCall()"]
...
- **Separate service** — agents run independently; the app works fully without AI (backend returns 503)
- **No-op tools** — the agent's 3 tools return their arguments as JSON. Item creation happens client-side after user approval.
- **Tool execution on frontend** — `useCopilotActions` maps tool calls to existing mutation hooks (`useCreateProject`, etc.)
- **German system prompt** — Copilot speaks German, helps organize tasks and projects, suggests but never executes
