...
# Flow: Job Search — CV Tailoring with Copilot

Upload a job description, let Copilot analyse it against your CV, create a tailored markdown version you can review and edit, then render a professional PDF — all within Senticor Project.
...
1. **Create a project** — Open Copilot and say: _"Erstelle ein Bewerbungsprojekt für die Rolle Product Manager bei Anthropic."_
   Copilot will suggest a project with actions. Accept the suggestion.
...
3. **Ask for analysis** — Say: _"Vergleiche meinen Lebenslauf mit der Stellenanzeige."_
   Copilot reads both documents, then lists strengths, gaps, and concrete suggestions.

4. **Request a tailored CV** — Say: _"Erstelle eine angepasste Version für diese Stelle."_
   Copilot creates a new markdown reference with the tailored CV content, linked to your project.
...
6. **Render PDF** — Say: _"Rendere die angepasste Version als PDF. Stil: modern, clean, Inter."_
   Copilot calls `render_cv` with the markdown reference + custom CSS. Accept the suggestion card.
...
graph LR
  ...
  A["<b>ANALYSE</b><br/>Copilot reads & compares"]
  T["<b>TAILOR</b><br/>Copilot creates markdown"]
  E["<b>EDIT</b><br/>User reviews & edits"]
  R["<b>RENDER</b><br/>PDF generated"]
  S["<b>SAVE</b><br/>Reference in Senticor Project"]
...
## Phase 2: Analyse — Copilot Reads Both Documents

**User story**: _"How well does my CV fit this job? What should I emphasise?"_

**Flow**:

1. User asks Copilot to compare CV and job description
2. Copilot calls `list_project_items(projectId)` → discovers item IDs
3. Copilot calls `read_item_content(jobDescriptionId)` → gets full text
4. Copilot calls `read_item_content(cvId)` → gets full text
5. Copilot streams an analysis: strengths, gaps, concrete suggestions
...
sequenceDiagram
  ...
  participant T as Copilot (Agent)
...
## Phase 3: Tailor — Copilot Creates a Markdown CV
...
1. Copilot reads the original CV via `read_item_content`
2. Copilot writes a **new tailored markdown CV** using `create_reference` with:
   - `description`: the full markdown content
   - `projectId`: linked to the application project
3. The original CV scopilots untouched — each tailored version is a new reference
...
1. User asks Copilot to render the tailored CV as PDF with style preferences
2. Copilot calls `render_cv` with `sourceItemId` (the markdown reference) + CSS + filename + projectId
...
graph TD
  ...
  G["Create reference<br/><b>CreativeWork in Senticor Project</b>"]
...
**`render_cv` tool call** (what Copilot produces):
...
- **"The summary is too long, shorten it."** → Edit the markdown reference directly, then ask Copilot to re-render
- **"Use Source Sans 3 instead of Inter."** → Copilot generates new CSS with `render_cv`, same `sourceItemId`
- **"Add my volunteer work."** → Edit the markdown, or ask Copilot to create a new version
- **"Make a version for the Berlin office."** → Copilot creates another tailored markdown + renders PDF
...
graph TD
  ...
  TCP["CopilotChatPanel<br/><i>conversation</i>"]
...
TSC["CopilotSuggestionCard<br/><i>review & accept</i>"]
TE["Tool Executor<br/><i>agents/tool_executor.py</i>"]
DR["Document Renderer<br/><i>Markdown → WeasyPrint</i>"]
FS["File Storage"]
REF["Reference Item<br/><i>CreativeWork in Senticor Project</i>"]
...
| **Agent tools**     | `agents/copilot.py`                             | `render_cv`, `create_reference`, `read_item_content`, `list_project_items`  |
...
| **Frontend hook**   | `frontend/src/hooks/use-copilot-actions.ts`     | Dispatches `render_cv` to backend                                           |
| **System prompt**   | `agents/prompts/de/copilot_system.j2`           | Two-step CV flow instructions for Copilot                                   |
...
Copilot:   [create_project_with_actions suggestion card]
...
Copilot:   [reads both documents via inline tools]
       Stärken:
...
Copilot:   [create_reference suggestion card — tailored markdown CV]
User:  ✓ Accept
       → New markdown reference created in Senticor Project

User:  [reviews rendered markdown in reference bucket, edits summary]

User:  Rendere als PDF. Modern, clean, Inter.
Copilot:   [render_cv suggestion card — filename + "from markdown reference"]
User:  ✓ Accept
       → PDF rendered from markdown, saved as reference in Senticor Project
```
