...
This document traces the complete GTD lifecycle through Terminandoyo's
...
```
   ┌──────────┐     ┌──────────┐     ┌──────────┐
   │ COLLECT  │────▶│ TRIAGE   │────▶│ ORGANIZE │
   │          │     │          │     │          │
   │ Inbox    │     │ Inline   │     │ Route to │
   │ Capture  │     │ actions  │     │ bucket   │
   └──────────┘     └──────────┘     └──────────┘
                                          │
                    ┌──────────┐           │
                    │  ENGAGE  │◀──────────┤
...
4. Input clears, item appears at bottom of inbox list (FIFO)
...
**Data created**: `InboxItem` with canonical ID `urn:gtd:inbox:{uuid}`
...
**User story**: "Time to process my inbox. The oldest item is first — I'll
...
1. Only the **first** (oldest) item in InboxList shows the triage action bar
...
- Bucket buttons use GTD bucket colors for instant visual recognition
...
| Next Actions  | `ActionList` (filtered: next) | Mark done, reschedule, archive |
| Projects      | `ProjectTree` (each project)  | Ensure each has a next action  |
| Waiting For   | `WaitingForList`              | Follow up on overdue items     |
| Someday/Maybe | `SomedayList`                 | Promote or archive             |
...
1. User opens ActionList (or taps BucketNav → Next Actions)
...
```
InboxCapture ──creates──▶ InboxItem
     │
     ▼
InboxList ──contains──▶ InboxTriage (inline on first item)
     │                      │
     │                      ├──bucket tap──▶ Action ──▶ ActionList
     │                      │                   │
     │                      │                   ├──▶ ProjectTree (if project assigned)
     │                      │                   ├──▶ WaitingForList (if waiting)
     │                      │                   └──▶ CalendarView (if calendar)
     │                      │
     ...
     │
     ▼
BucketNav ──navigates──▶ [all bucket views]
     │
     ▼
FocusList ──filters──▶ Action/Project (isFocused === true)
     │
     ▼
ReviewDashboard ──iterates──▶ [all bucket views in sequence]
     │
     ▼
GoalTracker ──links──▶ Projects + Actions (via refers_to)
```
