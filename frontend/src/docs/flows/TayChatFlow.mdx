...
**Component**: `TayChatToggle` (in `AppHeader`) → `TayChatPanel`
...
2. Clicks the button → `TayChatPanel` slides in from the right edge
...
**Components**: `TayMessageBubble` → `TaySuggestionCard`
...
- `TayTextMessage` — the conversational text
- `TaySuggestionMessage` — the tool call rendered as a card (status: `"pending"`)
...
**Components**: `TaySuggestionCard` → `useTayActions` → `TayConfirmation`
...
- **Tool execution on frontend** — `useTayActions` maps tool calls to existing mutation hooks (`useCreateProject`, etc.)
