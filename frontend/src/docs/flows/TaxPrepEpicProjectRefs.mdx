...
bucket — but with no project link. The user created a "2025 Tax Return" project
...
**Path A — Triage with project (during triage):**
...
**Path B — Assign project later (inline editor):**
In the reference list, expand a ReferenceRow → inline ItemEditor shows the
project picker. User selects a project → reference is linked.

**Path C — View in ProjectTree:**
...
│   │ ○ Download 1099-INT from Schwab        │
│   │ ● Download 1099-DIV from Vanguard  ✓   │
│   │                                         │
│   │ 📄 W-2 Form.pdf                        │
...
```
┌──────────────────────────────────────────────────────┐
│ ≡  📄 W-2 Form.pdf    triaged  pdf  2025 Tax Return │
└──────────────────────────────────────────────────────┘
```
...
mechanism works for CreativeWork — it's just a PropertyValue with a list of
...
```json
{
  ...
  "additionalProperty": [
    ...
    { "@type": "PropertyValue", "propertyID": "app:projectRefs", "value": ["urn:tay:project:abc"] }
  ]
}
```
...
Tay could auto-assign references to the correct project based on content:
...
This requires the project-reference linking to be in place first — Tay needs
