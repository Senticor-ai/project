...
The pipeline is defined in `.gitlab/ci.yml` (included from root `.gitlab-ci.yml`) and uses Kaniko jobs per image Dockerfile for this GitHub repository (`https://github.com/Senticor-ai/project`).
...
For this Senticor Project, CI uses Kaniko (no Docker daemon). The push jobs write `/kaniko/.docker/config.json` and publish images with immutable `CI_COMMIT_SHA` tags. The deploy job then updates `infra/k8s/overlays/production/kustomization.yaml` to that exact SHA on `main`.
