...
Push to main
  ...
  │
  └─ Reconcile ──── Flux CD detects changes in deploy/ directory
...
| Kubernetes manifests (deploy/) | Flux applies updated resources |
...
For this project, `docker:dind` is configured with `--insecure-registry=harbor.100-114-108-51.nip.io:30443` because Harbor uses an internal CA certificate that is not publicly trusted.
The CI job writes `~/.docker/config.json` from those two variables instead of calling `docker login`, so the Docker CLI does not fail TLS validation before the push step.
...
1. **Build stage** — `node:22-alpine`, runs `npm ci` and `npx vite build`
...
All application resources are declared in the `deploy/` directory using Kustomize. Flux injects the target namespace automatically — manifests must not set `namespace:`.

### Resources managed by the team (in `deploy/`)
...
### Resources managed by ops (not in `deploy/`)
...
For local Kubernetes development, use the manifests in `infra/k8s/minimal/` with Rancher Desktop. These are separate from the production `deploy/` manifests and use local image builds.
