...
project uses a **GitOps** deployment model. Code changes on `main` trigger an automated pipeline that builds container images, pushes them to a private registry, and lets a cluster-side reconciler apply the updated Kubernetes manifests. No manual `kubectl apply` is needed.
...
Two masked CI/CD variables must be configured in GitLab project settings:
...
The project has three Dockerfiles, all built from the monorepo root context:
