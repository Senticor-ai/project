...
This document is the testing reference for project. It covers test layers, isolation
...
The `chromium` Playwright project runs all tests **except** `*-llm.spec.ts`. The `llm` project runs only LLM E2E tests.
...
Tay chat scenarios are tested at two layers: **integration** (mocked LLM, deterministic)
...
| ---------------------------- | ----------------------- | ---------------------------------------------------- |
| **UI capture** (no reload)   | First captured (oldest) | Auto-expand fires when first item arrives; stays set |
