...
### Deterministic Google Workspace Harness

For inbox/calendar/copilot black-box flows, run the local mock harness and point backend API clients to it:
...
Seed the urgent schedule scenario with one command:
...
Canonical black-box regression test:
`backend/tests/test_google_workspace_mock_harness_e2e.py`.
...
The inbox sorts items by `provenance.createdAt` **descending** (newest first). Auto-expand
always picks the newest item (`sorted[0]`), and auto-advance after triage also picks the
newest remaining item. This is true regardless of how items were created (UI capture or API seed).
...
Without it, import jobs stay in "queued" status forever. If adding new async workflows that
...
### Capture Must Await Server Response

When creating items via the UI (not `apiSeed`), the frontend assigns a temporary `temp-*`
item ID for the optimistic cache entry. The server response replaces it with a real UUID.
If a subsequent action (triage, complete, etc.) fires before the response arrives, the PATCH
request targets the temp ID and the backend returns 500 (invalid UUID).

The `WorkspacePage.captureInboxItem()` method already handles this by calling
`page.waitForResponse()`. If you add similar capture helpers, always wait for the POST
...
### 401 After `page.reload()` (User-Agent Binding)

**Symptom:** `/items/sync` returns 401 after `page.reload()`, but `/auth/me` succeeds.

**Cause:** Playwright's `page.request` API (used by `ApiSeed`) and the browser page send
different User-Agent strings. If the backend has `SESSION_BIND_USER_AGENT=true` (default),
the session is invalidated when the UA changes between requests.

**Fix:** `.env.e2e` sets `SESSION_BIND_USER_AGENT=false`. If you see 401s after reload,
verify this setting is present. To diagnose which binding check fails, temporarily add
`logger.warning("auth.<reason>")` to `backend/app/deps.py:get_current_user()`.

### Service Logs

`e2e-stack.sh` redirects backend, worker, agents, and frontend output to
`frontend/test-results/logs/`. When a test fails, check these files for server-side errors:

```bash
# Check for 500s or stack traces
grep -i "error\|traceback\|500" frontend/test-results/logs/backend.log
```

### Debugging a Single Failing Test

```bash
# Run one test with headed browser + verbose output
bash scripts/e2e-stack.sh --headed --grep "capture → triage"

# Or run against an already-running stack
E2E_BASE_URL=http://localhost:5174 \
  npx playwright test --config e2e/playwright.config.ts --project=chromium \
  --grep "capture → triage" --headed
```
