...
<Mermaid chart={`
C4Context
  title System Context — project (TAY)
  ...
  System(tay, "project", "Structured task management with AI-assisted triage, Schema.org data model, append-only audit trail")
...
C4Container
  ...
  Rel(api, agents, "Proxies Tay chat requests", "HTTP")
...
- [Agents Service](?path=/docs/engineering-agents-service--docs) — Dual-agent architecture (Tay + OpenClaw), delegated auth, container environment
