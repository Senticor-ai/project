...
C4Container
  ...
  System_Boundary(tay, "TerminAndoYo") {
    ...
    Container(api, "Backend API", "Python, FastAPI", "REST API for items, files, imports, email, auth, search, push notifications")
    Container(worker, "Projection Worker", "Python", "Outbox consumer: search indexing, RDF projection, email sync, import jobs")
    ...
    ContainerDb(fuseki, "Apache Jena Fuseki", "RDF/SPARQL", "Relationship graph projection for FRBR/LexCEL queries (optional)")
  ...
  Rel(worker, fuseki, "Upserts RDF", "HTTP Graph Store")
  Rel(worker, gmail, "IMAP sync", "IMAP + OAuth")
  Rel(pushworker, pg, "Polls push_outbox", "SQL")
  Rel(api, openrouter, "Chat completions", "HTTPS")
...
C4Component
  ...
  Container_Boundary(worker, "Projection Worker") {
    ...
    Component(searchidx, "Search Indexer", "Python", "Indexes items and file content in Meilisearch. Handles OCR for PDFs.")
    Component(rdfproj, "RDF Projector", "Python", "Converts JSON-LD items to RDF triples. Validates via SHACL. Upserts to Fuseki graph store.")
