...
Who uses COPILOT and what external systems does it interact with?

<Mermaid chart={`
C4Context
  title System Context — Senticor Project (COPILOT)
...
System(copilot, "Senticor Project", "Structured task management with AI-assisted triage, Schema.org data model, append-only audit trail")

System_Ext(gmail, "Gmail", "IMAP email sync via OAuth 2.0")
System_Ext(openrouter, "OpenRouter", "LLM access for Copilot AI agent")
System_Ext(grafana, "Grafana LGTM", "Logs, traces, metrics via OpenTelemetry")

Rel_D(user, copilot, "Captures tasks, triages inbox, manages projects", "HTTPS")
Rel_D(cpa, copilot, "Receives exported document packages", "CSV/PDF/ZIP")
Rel_D(copilot, gmail, "Syncs email to inbox", "IMAP + OAuth")
Rel_D(copilot, openrouter, "AI-assisted triage and planning", "HTTPS/REST")
Rel_D(copilot, grafana, "Exports telemetry", "OTLP")
...
<Mermaid chart={`
C4Container
  title Container Diagram — Senticor Project

Person(user, "User", "Federal clerk")

System_Boundary(copilot, "Senticor Project") {
...
Container(agents, "Agents Service", "Python, Haystack, FastAPI", "Copilot AI assistant — chat completions with tool calling via OpenRouter (port 8002)")
Container(openclaw, "OpenClaw Runtime", "Per-user managed runtime (k8s Pod / local container)", "Autonomous AI agent — exec + curl skills, direct API access, self-discovers environment via env vars")
...
Rel(api, agents, "Proxies Copilot chat requests", "HTTP")
Rel(api, openclaw, "Proxies OpenClaw chat, writes JWT to /runtime/token", "HTTP + SSE")
Rel(openclaw, api, "Calls REST API via curl skills", "HTTP + delegated JWT")
Rel(openclaw, storybook, "Fetches docs for self-discovery", "HTTP")
Rel(agents, openrouter, "Chat completions", "HTTPS")
Rel(openclaw, openrouter, "Chat completions (user API key)", "HTTPS")
...
- [Agents Service](?path=/docs/engineering-agents-service--docs) — Dual-agent architecture (Copilot + OpenClaw), delegated auth, container environment
