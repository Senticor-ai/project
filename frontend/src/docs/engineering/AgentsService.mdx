...
OpenClaw is a self-hosted agent running in a per-user Docker container. Unlike Copilot, it acts
...
| ------------------------ | ---------------------------------- | -------------------------------------------------------------- |
| `COPILOT_BACKEND_URL`    | `http://host.docker.internal:8000` | Backend REST API (for all curl calls)                          |
| `COPILOT_FRONTEND_URL`   | `http://host.docker.internal:5173` | Frontend URL (context)                                         |
| `COPILOT_STORYBOOK_URL`  | `http://host.docker.internal:6006` | Storybook with product, design, and engineering docs           |
