...
Senticor Project supports two agent backends that share the same chat UI and backend APIs but are
...
| Aspect                     | Copilot (Haystack)                                          | OpenClaw                                   |
| -------------------------- | ----------------------------------------------------------- | ------------------------------------------ |
| **Backend**                | `agents/` service (port 8002), Haystack + OpenRouter        | Per-user runtime instance (k8s Pod by default) |
| **Execution model**        | Suggest → user approves → backend executes                  | Autonomous (exec + curl via skills)        |
| **System prompt**          | `agents/prompts/de/copilot_system.j2` (Jinja2, per-request) | `openclaw/workspace/AGENTS.md`             |
| **Conversation isolation** | `conversations.agent_backend = 'haystack'`                  | `conversations.agent_backend = 'openclaw'` |
| **UI identity**            | "Copilot" in header, avatar, greeting                       | "OpenClaw" in header, avatar, greeting     |
...
- **System prompts**: Copilot uses a Jinja2 template with dynamic user/time context;
  OpenClaw uses a static AGENTS.md in the container workspace.
- **Tool execution**: Copilot's tools are no-ops (suggest → approve). OpenClaw executes directly.

---

## Copilot (Haystack Agent)

The Copilot agent is a stateless FastAPI application (port 8002) using
[Haystack](https://haystack.deepset.ai/) with OpenRouter for LLM inference and tool calling.

**Core design principle**: The LLM only _suggests_ — it never creates items directly. Tool
...
├── copilot.py                    # Haystack Agent factory, tools, system prompt
...
├── prompts/
│   └── de/copilot_system.j2     # System prompt template (German, Jinja2)
└── tests/
...
| Claim        | Copilot             | OpenClaw                   | Purpose                                         |
| ------------ | ------------------- | -------------------------- | ----------------------------------------------- |
| `iss`        | `"project-backend"` | `"project-backend"`        | Issuer                                          |
| `aud`        | `"project-backend"` | `"project-backend"`        | Audience (self-issued)                          |
| `sub`        | User UUID           | User UUID                  | Item ownership                                  |
| `org`        | Org UUID            | Org UUID                   | Organization context                            |
| `act.sub`    | `"copilot"`         | `"openclaw"`               | Identifies the acting agent (audit trail)       |
...
Copilot's `tool_executor.py` sends `Authorization: Bearer {token}` plus `X-Agent: copilot`.
...
1. Build Senticor Project JSON-LD → `POST /items` → get `project_id`
...
- Capture source: `{"kind": "copilot", "conversationId": "..."}`
...
Copilot can see the user's workspace via read tools that call the backend API using a delegated JWT:

| Tool                      | Description                           | Backend endpoint           |
| ------------------------- | ------------------------------------- | -------------------------- |
| `list_workspace_overview` | Projects and item counts per bucket   | `GET /items/overview`      |
| `read_item_content`       | Fetch item data + extracted file text | `GET /items/:id`           |
| `list_project_items`      | List all items in a specific project  | `GET /items?projectId=:id` |

**Privacy boundary**: All read calls use a delegated JWT scoped to the current user's organization.
The agent cannot see items from other orgs.

---

## Conversation Management

Conversations are persisted in PostgreSQL so users can resume or review past chats.
...
| `user_id`       | `uuid`      | Owner                                        |
| `external_id`   | `uuid`      | Client-facing ID (used in API and frontend)  |
| `agent_backend` | `text`      | `'haystack'` or `'openclaw'` (isolation key) |
| `title`         | `text`      | Conversation title (nullable)                |
| `created_at`    | `timestamp` | Creation time                                |
| `updated_at`    | `timestamp` | Last message time                            |
| `archived_at`   | `timestamp` | Soft-delete timestamp (no-delete policy)     |
...
| `tool_calls`      | `jsonb`     | Tool call payloads (nullable)          |
...
| ------- | ---------------------------------- | ------------------------------------ |
| `GET`   | `/chat/conversations`              | List active conversations for user   |
| `GET`   | `/chat/conversations/:id/messages` | Get messages for a conversation      |
| `PATCH` | `/chat/conversations/:id/archive`  | Soft-delete (archive) a conversation |

### Multi-turn Context

Each completion request includes the last 50 messages from the active conversation
(`get_conversation_messages`). The frontend sends a `conversationId` per session;
the backend uses `get_or_create_conversation` to persist the conversation record.

### Frontend

- `conversationId` tracked per session in `useChatState`
- `ConversationList` component: list, resume, archive, and start new conversations
- History toggle button in `CopilotChatPanel` header

---

## Streaming Protocol (NDJSON)

Tokens stream as they arrive for a responsive feel. The backend proxies NDJSON from the agents
service to the frontend.
...
| --------------- | --------------------------- | ---------------------------------- |
| `text_delta`    | `{ text: string }`          | Incremental assistant content      |
| `tool_calls`    | `{ toolCalls: ToolCall[] }` | Tool call suggestions from the LLM |
| `auto_executed` | `{ toolName, result }`      | Read tool executed automatically   |
| `done`          | `{}`                        | Stream complete, finalize message  |

### Frontend Parsing

`readNdjsonStream` in `useCopilotApi` reads the `ReadableStream`, splits on newlines,
and dispatches events to `useChatState`. `CopilotMessageBubble` renders incrementally
via `isStreaming` flag — partial tokens appear as they arrive.

Tool call JSON is assembled from partial events and only executed after valid completion
(the `done` event).
...
| `OPENROUTER_API_KEY`          | _required_              | OpenRouter API authentication                         |
| `OPENROUTER_APP_URL`          | `""`                    | Sent as `HTTP-Referer` header to OpenRouter           |
| `OPENROUTER_APP_TITLE`        | `Senticor Project`      | Sent as `X-Title` header to OpenRouter                |
| `BACKEND_URL`                 | `http://localhost:8000` | Backend API base URL (for POST /items calls)          |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | _(unset)_               | OTLP collector endpoint; tracing disabled if absent   |

### Backend (for proxy)

| Variable                     | Default      | Description                                        |
| ---------------------------- | ------------ | -------------------------------------------------- |
| `AGENTS_URL`                 | _(unset)_    | Agents service URL; endpoints return 503 if absent |
...
OpenClaw is a self-hosted agent running per user as a managed runtime instance
(k8s Pod in deployed/local-k8s mode, local container in host-runtime mode).
Unlike Copilot, it acts autonomously — executing API calls directly via
`exec` + `curl` skills rather than suggesting and waiting for approval.

### Key differences from Copilot

- **No function-calling tools**: OpenClaw uses native exec + curl to call the backend API directly
- **No `POST /execute-tool`**: The execute-tool endpoint is Copilot-only (approval flow)
...
The backend injects environment variables into each OpenClaw runtime instance so
the agent can discover surrounding services. In k8s runtime these resolve to
cluster services (for example `http://backend:8000`), while host-runtime mode
rewrites localhost URLs to `host.docker.internal` via
`_to_container_url()` in `backend/app/container/manager.py`.

| Variable                 | Example (k8s runtime default) | Description                                                    |
| ------------------------ | ----------------------------- | -------------------------------------------------------------- |
| `COPILOT_BACKEND_URL`    | `http://backend:8000`         | Backend REST API (for all curl calls)                          |
| `COPILOT_FRONTEND_URL`   | `http://frontend`             | Frontend URL (context)                                         |
| `COPILOT_STORYBOOK_URL`  | `http://storybook`            | Storybook with product, design, and engineering docs           |
| `OPENCLAW_CONFIG_PATH`   | `/openclaw.json`                   | OpenClaw configuration file path                               |
| `OPENCLAW_GATEWAY_TOKEN` | _(generated)_                      | Gateway auth token for the container API                       |
| Provider API key         | _(decrypted from DB)_              | `OPENROUTER_API_KEY`, `OPENAI_API_KEY`, or `ANTHROPIC_API_KEY` |
...
- **API endpoints**: `curl $COPILOT_BACKEND_URL/openapi.json` — full OpenAPI spec (~112 KB, unauthenticated)
- **Product documentation**: `curl $COPILOT_STORYBOOK_URL/index.json` — Storybook story index
- **Environment check**: `echo $COPILOT_BACKEND_URL $COPILOT_FRONTEND_URL $COPILOT_STORYBOOK_URL`
...
| Variable | Default          | Description                              |
| -------- | ---------------- | ---------------------------------------- |
| Provider | _(user setting)_ | `openrouter`, `openai`, or `anthropic`   |
| API Key  | _(user setting)_ | User's own key (stored encrypted)        |
| Model    | _(user setting)_ | Model identifier for the chosen provider |
