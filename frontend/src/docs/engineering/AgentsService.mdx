...
Copilot's `tool_executor.py` sends `Authorization: Bearer {token}` plus `X-Agent: tay`.
...
- History toggle button in `TayChatPanel` header
...
`readNdjsonStream` in `useTayApi` reads the `ReadableStream`, splits on newlines,
and dispatches events to `useChatState`. `TayMessageBubble` renders incrementally
