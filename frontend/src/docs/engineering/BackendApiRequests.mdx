...
The exact JSON-LD shapes accepted by `POST /things` and `PATCH /things/{id}` are available as machine-readable JSON Schema via the backend API:
...
Available schemas: `inbox-thing`, `action-thing`, `project-thing`, `reference-thing`, `thing-patch`, `property-value`.
