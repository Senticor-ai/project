...
| ----------------- | ---------------------- | --------------------- |
| `inbox-thing`     | `InboxThingJsonLd`     | Inbox capture payload |
| `action-thing`    | `ActionThingJsonLd`    | Action payload        |
...
| `app:projectRefs`             | n/a (custom)                      | Thing → Project(s) | Many-to-many project membership via `additionalProperty`. Replaces the former `isPartOf`/`hasPart` bidirectional mapping which was semantically wrong (both properties have domain `CreativeWork`, not `Action`/`Organization`). Legacy `isPartOf` data is read via backward-compatible fallback in `fromJsonLd`. |
