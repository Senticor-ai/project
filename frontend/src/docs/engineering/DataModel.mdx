...
| `projectIds`           | `app:projectRefs` | `additionalProperty`                                | Thing → Project(s), many-to-many                                            |
...
| ----------------- | ------------------------------ | ----------------------------------------------------- | --------------------------------------------------------------------- |
| `Action`          | `Thing`                        | `inbox` / `next` / `waiting` / `calendar` / `someday` | `contexts`, `isFocused`, `delegatedTo`, `recurrence`, `sequenceOrder` |
| `DigitalDocument` | `Thing`                        | `inbox` (before triage)                               | `rawCapture` (set to `name` by `fromJsonLd` fallback)                 |
| `EmailMessage`    | `Thing`                        | `inbox` (before triage)                               | `rawCapture`, `extractableEntities` via additionalProperty            |
| `CreativeWork`    | `Thing` or `ReferenceMaterial` | `inbox` or `reference`                                | `url`, `encodingFormat`                                               |
...
as `Thing` with `bucket: "inbox"`. `CreativeWork` in inbox is also deserialized as `Thing`.
...
The serializer contract tests in `thing-serializer.test.ts` fetch these schemas, compile
...
| ----------------- | ---------------------- | ------------------------------------------------------- |
| `inbox-thing`     | `InboxThingJsonLd`     | Inbox capture payload (deprecated — use `action-thing`) |
| `action-thing`    | `ActionThingJsonLd`    | Action payload (inbox captures + triaged actions)       |
| `project-thing`   | `ProjectThingJsonLd`   | Project payload                                         |
| `reference-thing` | `ReferenceThingJsonLd` | Reference payload                                       |
| `event-thing`     | `EventThingJsonLd`     | Event payload                                           |
| `thing-patch`     | `ThingPatchModel`      | PATCH payload                                           |
...
| `Project` as pending type         | `Project` extends `Organization` (pending)         | Used as a top-level GTD type                                | `schema:Project` is a pending type not yet formally in the schema.org hierarchy. We use it because it directly expresses the GTD concept. The Organization relationship is enforced at the API layer via `org_id`. |
...
- Deprecate `inbox-thing` schema — inbox items now use `action-thing` (`@type: "Action"`)
