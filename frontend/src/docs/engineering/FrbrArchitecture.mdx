...
<Meta title="Engineering/Ontology/FRBR Knowledge Graph" />
...
implementation for the knowledge graph projection layer. FRBR extends the
[schema.org-first data model](?path=/docs/engineering-ontology--docs) with document
hierarchy concepts (Work, Expression, Manifestation, Item) that schema.org does not cover.

For typed references and ports, see
[LexCEL Architecture](?path=/docs/engineering-ontology-lexcel-architecture--docs).
For the workflow methodology, see
[Methodology](?path=/docs/product-methodology--docs).
