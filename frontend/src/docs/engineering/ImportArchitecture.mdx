...
| `transform.py`    | Nirvana JSON → project item conversion logic                                           |
...
<Mermaid chart={`
graph TD
  USF["User selects file"]
  USF --> HF

  subgraph ID ["ImportDialog"]
    HF["1. handleFile(file)"]
    HF --> UL["upload.mutateAsync(file)<br/><i>useFileUpload: initiate → chunks → complete</i>"]
    UL --> FR["FileRecord { file_id, sha256 }"]
    FR --> CD["checkDuplicate(sha256)<br/><i>optional</i>"]
    CD --> INS["inspect.mutateAsync()<br/><i>config.inspectFn → API → ImportSummary</i>"]

    INS --> REV["2. User reviews preview"]
    REV --> SI["startImport.mutateAsync()<br/><i>config.importFn → API → ImportJobResponse</i>"]

    SI --> POLL["3. Poll job status every 2s<br/><i>ImportsApi.getJob(jobId)</i>"]
    POLL --> INV["While running: invalidate items every 3s"]
    INV --> DONE["On completed: final invalidation"]

    DONE --> RES["4. Show results<br/><i>created / updated / skipped / errors</i>"]
  end

  style USF fill:#fff3e0,stroke:#ef6c00
  style HF fill:#e3f2fd,stroke:#1976d2
  style REV fill:#e3f2fd,stroke:#1976d2
  style POLL fill:#e3f2fd,stroke:#1976d2
...
2. **Transform** (`transform.py`): Write a function that converts the source format into project item dicts. Include bucket mapping and field normalization.
