...
Senticor Project can ingest emails from Gmail as inbox items via the Gmail API.
...
Archive actions are **bidirectional**: archiving a gmail item in COPILOT removes
it from the Gmail inbox, and archiving an email in Gmail archives the
corresponding item in COPILOT.
...
## Local Mock Harness Mode (Human Testing)

For deterministic UX testing without real Google credentials, run the local harness:
...
Then use the dev endpoint `POST /dev/mock-workspace/connection` (with `DEV_TOOLS_ENABLED=true`)
to provision a mock connection for the current user and trigger normal `/email/.../sync` flows.
Seed urgent scenario data via:
...
Decision gates in this flow are CEL-backed and centralized in:

- `backend/app/email/rules/sync_behavior_rules.json`

Those rules are evaluated by `backend/app/email/cel_rules.py` and applied in sync/proposal runtime code.
...
4. For each archived message (`labelRemoved` with INBOX): find the
   corresponding COPILOT item and set `archived_at`
...
automatically: active COPILOT items whose Gmail messages are no longer in the
inbox get archived.
...
| ------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| **COPILOT → Gmail** | User archives a gmail item in COPILOT | Worker calls `sync_email_archive()` → `gmail_api.message_modify(remove_label_ids=["UNREAD", "INBOX"])`  |
| **Gmail → COPILOT** | User archives an email in Gmail       | Next sync detects `labelsRemoved` (INBOX) in history → sets `archived_at` on corresponding COPILOT item |

**Loop prevention**: When COPILOT archives an item, the Gmail label change appears
...
cross-checks all active COPILOT gmail items against the current Gmail inbox. Items
...
4. Creates a service account (`gmail-push-sub@<your-gcp-project-id>.iam.gserviceaccount.com`)
5. Grants Subscriber role on the subscription
6. Downloads the service account JSON key to `tmp/<your-gcp-project-id>-pubsub-sa.json`
...
2. Create a Pub/Sub topic:
   ```bash
   gcloud pubsub topics create gmail-push --project=<your-gcp-project-id>
   ```
3. Grant Gmail permission to publish to the topic:
   ```bash
   gcloud pubsub topics add-iam-policy-binding projects/<your-gcp-project-id>/topics/gmail-push \
...
4. Create a pull subscription:
   ...
   gcloud pubsub subscriptions create gmail-push-sub \
     --topic=gmail-push --ack-deadline=30 --project=<your-gcp-project-id>
   ```
...
GMAIL_PUBSUB_CREDENTIALS_FILE=tmp/<your-gcp-project-id>-pubsub-sa.json
```
...
The integration defaults to least-privilege scopes:
...
The scope string can be overridden via `GMAIL_SCOPES`.
...
| `app/email/google_calendar_api.py` | Google Calendar API client (calendar list + event read/write)                                       |
| `app/email/calendar_sync.py`       | Calendar sync orchestrator (backfill + incremental per selected calendar)                           |
...
| `EmailConnectionCard` | Connection status, Gmail sync controls, and calendar opt-in/out per Google calendar |
...
| `backend/tests/test_calendar_sync.py`              | Calendar backfill/incremental sync, token reset, multi-calendar selection and per-calendar cursor persistence |
| `backend/tests/test_google_workspace_proposals.py` | Proposal generation + confirm flow with audit log persistence                                                 |
| `backend/tests/test_email_worker.py`               | Worker event handlers: sync job dispatch, COPILOT→Gmail archive, polling scheduler                            |
| `backend/tests/test_watch_worker.py`               | Pub/Sub notification processing, watch renewal                                                                |
| `frontend/.../EmailConnectionCard.test.tsx`        | Connection card UI: status badge, sync method indicator, calendar selection opt-in/out, actions               |
| `frontend/.../EmailPanel.test.tsx`                 | Panel: empty state, loading, connection list, callbacks (including calendar sync controls)                    |
