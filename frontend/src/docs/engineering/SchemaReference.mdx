...
are applied to app entities and which fields use `additionalProperty` for custom extensions.
...
Person (personal owner) or Organization (business owner)
    └── owns → Project
                  ├── contains → Task (Action subtypes: BuyAction, CreateAction, etc.)
...
The same schema.org-driven model can also represent complex public-sector workflows, such as grant or funding applications to nonprofits. This is a good signal for the depth of application and breadth of management the app can support, similar to broad "job to be done" task management tools.
...
**Schema:** `schema:Action` and action subtypes
**Purpose:** A unit of work that can be assigned, scheduled, and completed

The app supports schema.org action subtypes to provide semantic specificity while maintaining a simple UX. All action types share the same base properties but allow for specialized classification via `@type`.

#### Supported Action Subtypes

The app recognizes the following schema.org action types:

- `schema:Action` — Generic action (default)
- `schema:PlanAction` — Planned or future action
- `schema:BuyAction` — Purchase or procurement task
- `schema:CommunicateAction` — Communication task (email, call, message)
- `schema:ReviewAction` — Review or approval task
- `schema:CreateAction` — Content creation task (document, design, etc.)
- `schema:SendAction` — Send or deliver task
- `schema:CheckAction` — Verification or validation task
- `schema:ReadAction` — Reading or research task
...
interface Task extends Thing {
  ...
  description?: string;
  agent?: ThingRef<Person | Organization>; // Assignee (direct performer)
  ...
  object?: ThingRef | string; // What the action acts on
  result?: ThingRef | string; // What the action produces
  location?: ThingRef | string; // Where work happens
  instrument?: ThingRef | string; // Tool or device used
}
```

#### Relationship Properties

Action subtypes support schema.org relationship properties to capture semantic context:

- **`object`** — What the action acts on (e.g., "Apples" for a `BuyAction`, "Contract" for a `ReviewAction`)
- **`result`** — What the action produces (e.g., "Signed contract" for a `ReviewAction`)
- **`location`** — Where the action happens (e.g., "Supermarket" for a `BuyAction`)
- **`instrument`** — Tool or device used (e.g., "DocuSign" for a `ReviewAction`)
- **`agent`** — Direct performer of the action (assignee)
- **`participant`** — Additional collaborators or stakeholders

#### Example: BuyAction with Relationships

```json
{
  "@id": "urn:app:action:buy-apples",
  "@type": "schema:BuyAction",
  "name": "Buy apples for party",
  "description": "Get 5kg of apples from the supermarket",
  "agent": { "@id": "urn:app:person:john" },
  "object": "Apples",
  "location": "Edeka Supermarket",
  "result": "Purchased apples",
  "app:bucket": "personal",
  "actionStatus": "schema:PotentialActionStatus"
}
```

#### Example: CreateAction with Context

```json
{
  "@id": "urn:app:action:create-list",
  "@type": "schema:CreateAction",
  "name": "Create shopping list",
  "description": "Write shopping list for husband",
  "agent": { "@id": "urn:app:person:maria" },
  "result": { "@id": "urn:app:creative-work:shopping-list" },
  "instrument": "Notes app",
  "app:bucket": "personal",
...
1. **Capture** — All user inputs are created as `schema:Thing` (generic)
2. **Triage** — User or AI enrichment can reassign `@type` to a specific action subtype
3. **Enrichment** — AI pipeline can add relationship properties (`object`, `instrument`, etc.)
4. **Frontend** — Serializer recognizes all action subtypes and maps them to the action bucket
...
- [Data Model](?path=/docs/engineering-ontology--docs) - How schema.org types are applied to app entities
