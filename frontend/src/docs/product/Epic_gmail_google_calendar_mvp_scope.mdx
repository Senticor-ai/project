...
<Meta title="Product/Epics/Gmail + Google Calendar MVP Integration" />

# Epic: Gmail + Google Calendar MVP Integration

**Status:** Draft
**Priority:** Current
**Checklist audit:** February 23, 2026 — epic is still in draft planning; all acceptance criteria remain unchecked.
**Last updated:** 2026-02-23
**Depends on:** OAuth/auth foundation, connector runtime, search/normalization pipeline, Tay Copilot approval flow
...
**Outcome:** A single-user MVP where Gmail and Google Calendar data is ingested reliably, surfaced in Senticor context/search, and used by Tay to generate confirmation-gated proposals for time-sensitive workflows.

---

## User Story

> "I want to connect my Google account once, see fresh email/calendar context in Senticor, and let Tay suggest useful actions that only execute after I confirm."
...
- Tay proposal flows (confirmation required) for:
...
- Tay analyzes new relevant emails using nearby calendar context.
- Tay outputs either `NoAction` or a proposal payload.
...
- Optional only when needed: `https://www.googleapis.com/auth/calendar`
...
- **Agent tooling layer:** deterministic Gmail/Calendar tools invoked by Tay.
...
2. Tay detects reschedule intent and proposes time shift.
...
1. Tay extracts requested time/task from email.
2. Tay proposes calendar reminder/event + draft reply.
...
- [ ] User can connect Google account and recover via reconnect flow.
- [ ] First sync completes and exposes Gmail + Calendar context with freshness timestamp.
- [ ] Incremental polling updates new Gmail messages and relevant calendar changes.
- [ ] Tay generates proposal cards for both target workflows.
- [ ] No write action executes without explicit confirmation.
- [ ] Confirmed actions correctly call Google APIs and persist audit records.
- [ ] Core connector metrics and failure alerts are visible.
