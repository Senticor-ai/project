...
- [x] `project-cli/SKILL.md` — CLI command reference
...
### Slice 2: DevContainer + Project Mount

Create devcontainer config. Update container manager to mount the project repo.

- [ ] `openclaw/.devcontainer/devcontainer.json` — devcontainer spec
- [ ] `openclaw/.devcontainer/Dockerfile` — image definition
- [ ] `backend/app/container/manager.py` — mount project, use devcontainer image
- [ ] `backend/app/container/workspace.py` — provision coding skill deps

### Slice 3: Default to OpenClaw

Update backend chat routing to default all users to OpenClaw.

- [ ] `backend/app/chat/routes.py` — default backend selection
- [ ] Feature flag or setting for rollback to Haystack

### Slice 4: Haystack Interim Fix (done)
