...
- _Speed over ceremony_ — reducing taps from 3+ to 1 for energy/time editing
- _Everything is an object_ — metadata chips are visible, tappable objects on the item row
- _Relationships over navigation_ — metadata is edited in-place, not behind a separate form
