...
<Meta title="Product/Completed Epics/Org Knowledge & Person Items" />

# Epic: Org Knowledge Documents, Person Items, and Agent Org Awareness

**Status:** Closed (Features 1 & 2 shipped; Feature 3 deferred to Platform SDK)
...
Issues #54, #55, #57 closed February 25, 2026:

- **Backend content endpoints** — `PATCH /items/{id}/file-content` and `POST /items/{id}/append-content`
  for inline markdown editing of org knowledge documents (content stored in `schema_jsonld.text`)
- **PersonItem** — `@type: "Person"` reference items with email, telephone, jobTitle, orgRole;
  `isPersonItem()` type guard; full `toJsonLd`/`fromJsonLd` roundtrip; `PersonRow` component
  with role badges, org badge, mailto/tel links
- **OrgDocItem** — `OrgDocType` (`general | user | log | agent`); `isOrgDocItem()` guard;
  `OrgDocEditor` component (editable textarea for GENERAL/USER, append-only log, read-only AGENT);
  special icons + i18n subtitles in `ReferenceRow`; org docs sort first within each org group
- **Testing** — Storybook stories with MSW handlers, vitest unit + storybook coverage (all green)

## What was deferred

Issue #56 (**Org-aware Copilot flows**) — agent tools (`list_organizations`, `read_org_document`,
`update_agent_notes`, `append_org_log`, `create_person`), `BackendClient` methods, system prompt —
remain open and will be implemented as part of the Platform SDK & Agent CLI epic.
...
| Agents   | `agents/copilot.py`                       | 5 new tools, org context in prompt             |
...
| Agents   | `agents/prompts/de/copilot_system.j2`     | Org awareness section                          |
...
individual Python tools. See the [Platform SDK & Agent CLI](?path=/docs/product-completed-epics-platform-sdk-agent-cli--docs)
epic for the full architecture.

**Key decisions:**

- **Scopilot with PostgreSQL.** JSON-LD JSONB is sufficient. No triple store (Fuseki/SPARQL) needed —
...
- **CLI over many tools.** One `copilot_cli` tool replaces 7+ Python tools. Same CLI serves Copilot, backend
