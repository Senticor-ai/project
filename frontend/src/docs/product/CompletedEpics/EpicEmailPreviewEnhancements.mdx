...
<Meta title="Product/Completed Epics/Email Preview Enhancements" />
...
email preview — where the user's attention is focused — has no archive affordance.

### Solution

Add an "Archivieren" button to the `EmailBodyViewer` footer, next to the
"In Gmail öffnen" link. The button triggers the existing `onArchive` callback
...
│                                                      │
├──────────────────────────────────────────────────────┤
│ 🔗 In Gmail öffnen              [📥 Archivieren]     │
└──────────────────────────────────────────────────────┘
```

**Behavior:**

- Button calls `onArchive()` — the same callback already used by `ActionRow`
...
- Bidirectional sync is already handled — archiving in COPILOT removes the
...
export interface EmailBodyViewerProps {
  ...
  senderEmail?: string;
  className?: string;
  onArchive?: () => void; // NEW — archive callback from parent
}
```
...
**3a) CSS containment** — Add overflow and responsive styles to the email body
wrapper:
...
**3b) Mobile layout adjustments** — On small screens (`< md`), adjust the
...
- Footer stacks: "In Gmail öffnen" link above, archive button below
...
**3c) DOMPurify post-processing** — After sanitization, strip inline `width`
...
- [x] Expanded email body text is selectable with standard browser selection
- [x] "Kopieren" button appears in header when email body is expanded
- [x] Clicking "Kopieren" copies the email body as clean plaintext to clipboard
- [x] Button shows "Kopiert!" feedback for 2 seconds after successful copy
- [x] Copy works in Chrome, Firefox, and Safari

### Archive

- [x] "Archivieren" button appears in email preview footer when `onArchive` is provided
- [x] Clicking "Archivieren" triggers the same archive flow as the overflow menu
- [x] Button is not shown for already-archived items or contexts without archive support
- [x] Bidirectional Gmail sync works (COPILOT archive removes Gmail INBOX label)

### Responsive

- [x] Email HTML tables do not overflow the container on 375px-wide viewport
- [x] Email images scale down to fit container width
- [x] Inline `width: NNNpx` styles on tables/divs are stripped during sanitization
- [x] Long unbroken strings (URLs) wrap instead of overflowing
- [x] Header layout stacks vertically on screens below `md` breakpoint
- [x] Footer actions stack vertically on screens below `md` breakpoint
- [x] Email preview is usable on iPhone SE (375px) and Galaxy S8 (360px) viewports
...
- [x] Add overflow/containment CSS to email body wrapper
- [x] Add width-stripping post-processing to DOMPurify output
- [x] Add responsive header/footer layout (stacking on mobile)
- [x] Add Storybook stories for mobile viewport and wide-table email
- [x] Add unit tests for width stripping and overflow behavior

### Phase 2: Copy/paste support

- [x] Add plaintext extraction utility (sanitized HTML → textContent)
- [x] Add "Kopieren" button to email header (expanded state only)
- [x] Implement clipboard write with fallback
- [x] Add "Kopiert!" feedback animation
- [x] Add unit tests and Storybook story for copy interaction

### Phase 3: Archive from preview

- [x] Add `onArchive` prop to `EmailBodyViewerProps`
- [x] Render "Archivieren" button in footer when prop is provided
- [x] Wire `onArchive` from `ActionRow` through to `EmailBodyViewer`
- [x] Add unit tests for archive button visibility and callback
- [x] Add Storybook story with archive action
...
5. Manual: expand an email, copy text, paste into a text editor — confirm clean plaintext
