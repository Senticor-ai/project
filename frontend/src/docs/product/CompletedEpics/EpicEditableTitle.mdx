...
<Meta title="Product/Completed Epics/Editable Title & Rename Provenance" />

# Epic: Editable Title & Rename Provenance

**Status:** Complete
**Priority:** Current
**Progress note:** Frontend delivered on February 23, 2026.

## Context

Items have two text identities: `rawCapture` (the original verbatim input) and `name` (a deliberate, possibly AI-suggested title). Currently the UI conflates them — there is no way to see or edit them separately, and no record of who changed the name or why.
...
**Design goal:** Users and AI can both rename items, and provenance makes it clear who did what. The original capture text is always preserved and visible.
...
Optional `name_source` field on `ItemPatchRequest` overrides the derived source string.

**Files:** `backend/app/routes/items.py`, `backend/app/models.py`

### DB indexes

Three partial indexes on `schema_jsonld` (filtered `WHERE archived_at IS NULL`):

- `idx_items_name` — B-tree on `(org_id, name)` for exact/prefix lookups
- `idx_items_name_trgm` — GIN trigram for ILIKE/fuzzy search
- `idx_items_jsonld_gin` — GIN `jsonb_path_ops` for containment queries (covers `rawCapture`)
...
**Files:** `backend/tests/test_items_patch_delete.py`, `backend/tests/test_openapi_items_schema.py`
...
`frontend/src/lib/item-serializer.test.ts`:

- Roundtrip: `app:nameProvenance` survives `toJsonLd` / `fromJsonLd`
- `fromJsonLd` extracts `nameProvenance` into the ActionItem model

`frontend/src/model/types.ts`:

- Add `NameProvenance` interface: `{ setBy: "user" | "ai" | "system", setAt: string, source: string }`
- Add optional `nameProvenance?: NameProvenance` to `ActionItem` base type
...
### Phase 3: Integration into ActionRow / expanded editor

#### 3a. RED — Update ActionRow tests

- ActionRow renders `EditableTitle` in expanded mode
- ActionRow passes `onRename` that calls `ItemsApi.update` with `name_source`

#### 3b. GREEN — Wire up

- Replace inline title rendering in ActionRow expanded view with `<EditableTitle>`
- Connect `onRename` to the PATCH API via `useItems` hook
...
- `frontend/src/lib/item-serializer.ts` — serialize/deserialize `nameProvenance`
- `frontend/src/lib/item-serializer.test.ts` — roundtrip tests
- `frontend/src/components/work/ActionRow.tsx` — integrate `EditableTitle`
- `frontend/src/components/work/ActionRow.stories.tsx` — add renamed variants
- `frontend/src/lib/api-client.ts` — optional `nameSource` param on `ItemsApi.update`

---

## Verification checklist

- [x] `nameProvenance` roundtrips through serializer
- [x] EditableTitle renders name vs rawCapture separately
- [x] Provenance badge shows AI vs user attribution
- [x] Rename calls PATCH with `name_source`
- [x] Multiple renames accumulate in provenance history
