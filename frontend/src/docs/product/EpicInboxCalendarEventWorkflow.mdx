...
**Priority:** Current
**Checklist audit:** February 26, 2026 -- all acceptance criteria verified against implementation; 20/21 done, 1 N/A (migration not needed in alpha).
...
- Migration/compat logic for existing calendar `Action` items (N/A in alpha; runtime `mergeCalendarEvents()` fallback sufficient).
...
- [x] Enabling calendar sync makes current-day and future synced events visible in Project calendar views.
- [x] Inbox `Move to Calendar` persists `@type: "Event"` with required date semantics.
- [x] Calendar supports List and Grid (Month + Week) with parity for core event actions.
- [x] Project filter works in both modes with `Any selected` semantics for multi-project events.
- [x] User can accept/tentative/decline eligible events in Project and changes propagate to Google.
- [x] User can reschedule writable events in Project and changes propagate to Google.
- [x] User can delete writable events in Project and deletion propagates to Google.
- [x] Non-writable events show correct disabled states and explanatory feedback.
- [x] Local and provider sync states are visible (`Synced`, `Saving`, `Sync failed`, `Local only`).
- [x] Copilot proposal generation is automatically triggered by new email/event sync signals (no manual generate step required for normal flow).
- [x] Urgent proposals generate browser notifications (or in-app high-priority fallback when notification permission is unavailable).
- [x] Urgent proposal pipeline uses unified notification envelope and deduplicates by `event_id`.
- [x] Agent runtime can be woken via backend SSE notification stream without direct Postgres access.
- [x] CLI supports stream watching and notification send commands via the same backend notification pipeline.
- [x] Busy agent does not cause missed proposal candidates; queued work is processed after capacity returns.
- [x] Lease timeout recovers stuck agent tasks and re-queues candidate processing automatically.
- [x] Dead-lettered candidate tasks are visible with error diagnostics and retry metadata.
- [x] Confirming urgent reschedule proposal updates Google Calendar and sends confirmation email reply.
- [x] Confirming urgent personal-request proposal performs mapped calendar/email actions with audit log.
- [x] Existing calendar `Action` items are migrated (or compatibly rendered) without data loss. _(N/A in alpha — runtime `mergeCalendarEvents()` fallback handles edge cases.)_
- [x] Audit logs capture all provider write attempts and outcomes.
