...
# Epic: Tay Copilot V1 — Chat + Item Creation

**Status:** V1 complete (frontend + backend + agents)
**Priority:** Current

## Context

Tay is the AI assistant embedded in project ([Product Vision](?path=/docs/product-vision--docs#tay--the-ai-assistant)). Until now, Tay existed only as a documented concept. This epic delivers V1: a conversational side panel where users chat with Tay and Tay can create items on the user's behalf — but **only after explicit approval**.
...
| Interaction model | Suggestions as detachable attachments — never auto-applied |
...
1. **Chat** — free-form text conversation
2. **Create a project with actions** — suggest a project name, desired outcome, and a list of actions. Optionally include a DigitalDocument (e.g. invitation template).
3. **Create a single action** — suggest an action for any bucket, optionally linked to an existing project.
4. **Create a reference** — suggest a reference material (document, URL, note).
...
The chat lives in a **right side panel** — the first implementation of the planned [Sheet / Side Panel](?path=/docs/design-paperclip-principles--docs) widget.
...
**State:** `isChatOpen: boolean` in `AuthenticatedApp`. No routing changes — chat is ephemeral UI state.
...
┌──────────────────────────────────────────────┐
│ ✦ Projekt: Geburtstagsfeier planen           │
│   Ergebnis: Erfolgreiche Geburtstagsfeier    │
│                                              │
│   Aktionen:                                  │
...
- [x] `POST /chat/completions` — backend proxy to agents service (7 tests)
- [x] Agents service (`agents/`) — separate FastAPI on port 8002 (4 tests)
...
**Deferred to V2+:** Conversation history, user items in prompt context, streaming (SSE), model fallback, K8s agent manifests, CI pipeline jobs, DB persistence for conversations. See [Tay Copilot V2+](?path=/docs/product-epics-tay-copilot-v2---docs).
