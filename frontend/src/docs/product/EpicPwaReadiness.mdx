...
A gap analysis against the Procedere team's PWA-readiness checklist confirmed that project's core SPA architecture is solid: React Query hooks provide a swappable repository abstraction, optimistic mutations with rollback are universal, the OfflineBanner + `offlineFirst` network mode handle connectivity state, and the backend offers cursor-based sync with ETags and idempotent writes.

The remaining gaps before project can become an installable, offline-capable PWA fall into four phases:
...
**Design goal:** Users can install project on mobile, work offline, and have changes sync automatically when reconnected — without any UI component rewrites.
