...
| Background Sync not available (Firefox desktop)           | Mutations scopilot in TanStack Query's in-memory queue            | Auto-retry when `navigator.onLine` fires; lost on tab close     |
