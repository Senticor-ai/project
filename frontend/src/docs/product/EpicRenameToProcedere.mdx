...
Full rebrand from "project" (COPILOT) to "Procedere". The "Copilot" persona is removed (never shipped). Cookie names, DB names, and k8s namespace all change.
...
2. `frontend/src/App.tsx`:
   ...
   - `src="/tay-logo.svg"` → `src="/logo.svg"`, `alt="COPILOT"` → `alt=\{APP_NAME\}`
   - Hardcoded `project` → `\{APP_NAME\}`
...
- `frontend/e2e/fixtures/auth.fixture.ts` — `text=project` → `text=Procedere`
- `frontend/e2e/tests/nirvana-import.spec.ts` — same

### 2b. GREEN — Update MDX docs and stories

1. `frontend/src/docs/design/DesignPhilosophy.mdx` — `project (COPILOT)` → `Procedere`, table refs
2. `frontend/src/docs/product/ProductVision.mdx` — title, body, remove Copilot agent section entirely
...
- `backend/tests/test_auth_refresh.py` — cookie name `"project_session"` → `"procedere_session"`
- `backend/tests/conftest.py` — DB defaults `"project"` → `"procedere"`
...
1. `backend/app/config.py`:
   ...
   - DB user default: `"project"` → `"procedere"`
   - DB name default: `"project"` → `"procedere"`
   - Cookie defaults: `project_*` → `procedere_*`

2. `backend/app/main.py` — `title="project API"` → `title=f"\{settings.app_name\} API"`
...
- `.env.example` — cookie names `project_*` → `procedere_*`, add `APP_NAME=Procedere` and `VITE_APP_NAME=Procedere`
- `.env` (local) — update DB user, DB name, DATABASE_URL, cookie names

### 4b. Docker

- `infra/docker-compose.yml` — defaults `project` → `procedere`

### 4c. Kubernetes (environment-aware)

All files in `infra/k8s/` (base + overlays) — namespace `project` → `procedere-dev`, images `project-*` → `procedere-*`:
...
- `.claude/projects/` MEMORY.md — update all "project"/"COPILOT"/"Copilot" → "Procedere"

### 4f. VERIFY

```bash
# Final sweep for stale references
grep -ri "project|project|tay-logo" \
...
- Rename directory: `mv ~/workspace/project ~/workspace/procedere`
