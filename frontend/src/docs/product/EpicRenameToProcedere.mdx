...
Full rebrand from "project" (TAY) to "Procedere". The "Tay" persona is removed (never shipped). Cookie names, DB names, and k8s namespace all change.
...
1. `frontend/src/docs/design/DesignPhilosophy.mdx` — `project (TAY)` → `Procedere`, table refs
...
- `.claude/projects/` MEMORY.md — update all "project"/"TAY"/"Tay" → "Procedere"
