...
All files in `infra/k8s/minimal/` — namespace `terminandoyo` → `procedere-dev`, images `terminandoyo-*` → `procedere-*`:
namespace.yaml, configmap.yaml, secret.yaml, postgres.yaml, backend.yaml, frontend.yaml, pvc-backend-files.yaml, ingress.yaml, job-db-init.yaml
