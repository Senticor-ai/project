...
**Status:** In progress (backend APIs + CLI/SDK collaboration contract complete; board UI slices pending)
**Last updated:** 2026-02-26
**Checklist audit:** February 24, 2026 — checked items reflect implemented backend/API/CLI slices; remaining work stays unchecked.
**Technical spec:** February 26, 2026 — Appendix B added with frontend kanban architecture informed by senticor-flow reference analysis.
**Priority:** Current
...
- [ ] **Step 1 — Data layer:** `board-types.ts` + `collaboration-api.ts` + `use-project-actions.ts` + `use-board-mutations.ts`
- [ ] **Step 2 — Read-only board:** `BoardView` + `BoardColumn` + `BoardCard` (render actions grouped by `actionStatus`)
- [ ] **Step 3 — View toggle + routing:** `ProjectViewToggle` + `route-utils.ts` extension + deep links
- [ ] **Step 4 — Drag-and-drop:** cross-column transitions + within-column reorder + `BoardDragOverlay`
- [ ] **Step 5 — Inline editing:** title, assignee picker, due date picker, tag input on cards
- [ ] **Step 6 — Quick-add:** per-column card creation with title input
- [ ] **Step 7 — Context menu + keyboard:** `BoardCardContextMenu` + keyboard sensor + a11y announcements
- [ ] **Step 8 — Detail panel:** `ActionDetailPanel` (double-click) with markdown editor, comments, history
