...
<Meta title="Product/ThinkingRock Features" />

# ThinkingRock Feature Map

This document maps every ThinkingRock capability to Terminandoyo's Paperclip components.
Each feature is categorized by GTD phase and specifies the exact component, interaction
pattern, and data model backing.
...
| Delegation / waiting-for | **Spec** | `WaitingForList`   | 5      |
| Recurrence editor        | **Spec** | `RecurrenceEditor` | 6      |
| Weekly review            | **Spec** | `ReviewDashboard`  | 6      |
| Reference library        | **Spec** | `ReferenceLibrary` | 5      |
| Someday/maybe list       | **Spec** | `SomedayList`      | 5      |
...
**InboxCapture** (`src/components/gtd/InboxCapture.tsx`)
...
**InboxList** (`src/components/gtd/InboxList.tsx`)

- FIFO order: oldest item at top, only first item shows triage actions
...
**InboxTriage** (`src/components/gtd/InboxTriage.tsx`)

- Inline action bar on each inbox item card (no multi-step dialog)
- One-tap bucket assignment: Next, Waiting, Calendar, Someday, Reference, Archive
...
**Location**: `src/components/gtd/ProjectTree.tsx`

ThinkingRock's most distinctive feature: unlimited project/sub-project nesting
...
| --------------------- | ------------------------------------------------------------------------------ |
| Auto-sequence         | When action N completes, action N+1 auto-promotes to Next Actions              |
| Next action indicator | Only `sequenceOrder === 0` (or lowest incomplete) visible in Next Actions list |
...
**Location**: `src/components/gtd/ActionList.tsx`

ThinkingRock's "different views to display your tasks" — filtered, tabbed, customizable.
...
**Filter dimensions** (ThinkingRock's columns):

| Dimension      | Source                                | UI                                                |
| -------------- | ------------------------------------- | ------------------------------------------------- |
| Context        | `Action.contexts` → `GtdContext.name` | Chip toggle bar                                   |
...
**Location**: `src/components/gtd/ContextFilterBar.tsx`

The "what can I do right now?" filter — ThinkingRock's context-switching feature.
...
**Location**: `src/components/gtd/FocusList.tsx`

NirvanaHQ's "Focus" concept + ThinkingRock's goal alignment.
...
**Location**: `src/components/gtd/GoalTracker.tsx`

ThinkingRock's "Review your values and life priorities" feature.
...
```typescript
interface Goal extends GtdBaseEntity {
  bucket: "focus";
...
**Location**: `src/components/gtd/WaitingForList.tsx`

ThinkingRock's delegation tracking.
...
**Location**: `src/components/gtd/CalendarView.tsx`
...
**Location**: `src/components/gtd/RecurrenceEditor.tsx`

Set up recurring patterns (ThinkingRock's recurrent actions).
...
**Location**: `src/components/gtd/ReviewDashboard.tsx`

The weekly review is GTD's backbone. ThinkingRock enforces it; we make it delightful.
...
**Review steps** (matching GTD methodology):

| Step             | What                    | Actions Available                                 |
| ---------------- | ----------------------- | ------------------------------------------------- |
| 1. Clear Inbox   | Process remaining items | Triage each (inline triage on each card)          |
| 2. Next Actions  | Review each action      | Complete / Reschedule / Move to Someday / Archive |
| 3. Projects      | Check each project      | Has next action? Stalled? Complete?               |
| 4. Waiting For   | Follow up on delegated  | Follow up / Still waiting / Complete              |
| 5. Someday/Maybe | Incubation check        | Promote to Next / Keep / Archive                  |
...
- `WaitingForStep` — overdue follow-ups in red
...
**Location**: `src/components/gtd/ReferenceLibrary.tsx`

ThinkingRock's "Find information easily and quickly."
...
## 9. Someday/Maybe — "Have time for your someday/maybe list"

### Spec: SomedayList

**Location**: `src/components/gtd/SomedayList.tsx`

The incubation list — reviewed weekly, occasionally promoted.

**Paperclip design**:

- ObjectCards with `bucket="someday"` in a relaxed, airy layout (more spacing)
- Cards slightly desaturated (lower opacity on bucket badge)
- "Promote to Next" button on each card (snaps item to Next Actions)
...
All new components follow the Paperclip design language:
...
All ThinkingRock features map to existing types in `src/model/gtd-types.ts`:
...
| Contexts          | `Action.contexts: CanonicalId[]` + `GtdContext`                    |
...
| Someday/maybe     | `Action.bucket: "someday"`                                         |
...
| ------ | -------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| **5**  | `ActionList`, `ContextFilterBar`, `FocusList`, `WaitingForList`, `SomedayList`, `ReferenceLibrary` | Core "Engage" phase — users need to work from lists   |
