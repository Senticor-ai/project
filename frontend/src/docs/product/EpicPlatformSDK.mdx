...
**Status:** In progress (Phase 1 complete, Phase 2/3 partially implemented, dogfooding hardening in progress)
**Checklist audit:** February 23, 2026 — completed phases are checked; remaining hardening/dogfooding stays unchecked.
...
```txt
agents/tay.py      → 3 tool definitions (create_project_with_actions, create_action, create_reference)
...
- [ ] Full weekly planning/scheduling/review dogfooding loop validated with only Senticor Project as tracker
...
| Agents   | `agents/tay.py`                        | Replace 3 tools with single `copilot_cli` tool (legacy filename) |
...
6. Agents (post-refactor parity): `cd agents && uv run python -m pytest tests/` (CLI subprocess integration expectations)
7. CLI contract tests: `npm run copilot -- items list --summary --json` returns valid `copilot.v1` envelope (stdout-only JSON, non-zero errors on stderr)
8. CLI project+write flow tests: `npm run copilot -- projects create --name="Senticor Project" --description="Self-hosted dogfooding workspace" --propose --json`, then `npm run copilot -- items create --type=Action --name="Plan weekly review" --bucket=next --project=<project_id> --propose --json`, then `npm run copilot -- proposals apply <id> --yes --json`
9. CLI auth/non-interactive tests: missing token in CI mode fails with exit code `3` and structured error JSON
10. Manual E2E: Start dev stack → open Copilot chat → ask "Was gibt es im Workspace?" → Copilot calls `copilot_cli(argv=["items","list","--summary","--json"])` → returns workspace overview
11. Dogfooding E2E: Using only `copilot_cli`, create/update epic todos and references in active projects, then confirm state is visible in Senticor Project UI
