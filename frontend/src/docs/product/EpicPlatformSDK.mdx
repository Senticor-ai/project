...
**Outcome:** A TypeScript `@terminandoyo/core` package with CLI commands, serializers, API client,
...
| --- | --- | --- |
| **Frontend** (React app) | Browser | `import { fromJsonLd, toJsonLd } from "@terminandoyo/core"` |
| **Tay** (agent tool) | Node.js subprocess | `npx tay items list --bucket=reference` |
| **3rd party** | Browser or Node.js | `npm install @terminandoyo/core` |
...
| Frontend | `frontend/src/lib/item-serializer.ts` | Re-export from `@terminandoyo/core` |
| Frontend | `frontend/src/lib/api-client.ts` | Re-export from `@terminandoyo/core` |
