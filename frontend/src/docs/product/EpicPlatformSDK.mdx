...
| -------------------- | --------------------------------------------- | --------------------------------------------------------------- |
| **Read** (inline)    | Executes immediately, returns data to Copilot | `items list --bucket=reference`, `orgs get nueva-tierra --docs` |
...
8. Manual E2E: Start dev stack → open Copilot chat → ask "Was gibt es im Workspace?" → Copilot calls `tay_cli("items list --summary")` → returns workspace overview
