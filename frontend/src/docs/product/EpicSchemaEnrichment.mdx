...
**Goal:** Backend accepts schema.org action subtypes and relationship properties. Capture scopilots as `Thing`. User actions (triage/PATCH) or AI enrichment can reassign `@type` to a richer subtype. Frontend scopilots simple but doesn't break on new types.
