...
### 1.4 Switch `ThingJsonLd` union to custom discriminator
...
### 1.5 Update `ThingPatchModel`

- Change `type` from `Literal[...]` to `str | None`
- Add the same relationship properties as `ActionThingJsonLd`
...
### 2.1 Update `_is_action_type` in `things.py`
...
| `backend/app/routes/things.py` | `_is_action_type()` uses `ACTION_SUBTYPES` |
| `backend/tests/test_openapi_things_schema.py` | Update assertions for subtypes |
| `frontend/src/lib/thing-serializer.ts` | `fromJsonLd` recognizes subtypes |
| `frontend/src/test/msw/fixtures.ts` | `createThingRecord` handles subtypes |

---

## Verification checklist

- [ ] POST a `BuyAction` with `object` to `POST /things` -- stores and round-trips
