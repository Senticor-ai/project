...
**See also:** [Tay Copilot V2+](?path=/docs/product-epics-tay-copilot-v2---docs)
...
containers** that both human and AI (Tay) can work with.

**Problem**: Tay has no context about the user's organizations. Wolfgang manages 3 entities
...
**Outcome**: Each org gets 4 auto-created markdown documents (GENERAL, USER, LOG, AGENT). Tay gets
...
| **AGENT.md** | Tay's working notes and scratch space per org | Agent read/write, Human read-only |
...
```json
{
  ...
  "additionalProperty": [
    ...
    { "@type": "PropertyValue", "propertyID": "app:orgRef", "value": "{\"id\":\"org-nueva\",\"name\":\"Nueva Tierra\"}" },
    { "@type": "PropertyValue", "propertyID": "app:orgRole", "value": "accountant" }
  ]
}
```
...
## Feature 3: Agent (Tay) Org Awareness
...
Tay uses AGENT.md as persistent per-org memory:
1. On first interaction about an org, Tay reads GENERAL.md + USER.md + AGENT.md
2. After learning something new, Tay updates AGENT.md with structured notes
...
- **CLI over many tools.** One `tay_cli` tool replaces 7+ Python tools. Same CLI serves Tay, backend
  automation, and 3rd party integrations.
- **Isomorphic TypeScript.** Serializers + client + validation run in browser, Node.js, and as Tay's
...
7. Manual E2E: Create org in Settings &rarr; 4 docs appear in Reference bucket &rarr; edit GENERAL.md
   &rarr; ask Tay about the org &rarr; Tay reads org context &rarr; Tay appends to LOG.md
