...
<Meta title="Product/Epics/Tay Copilot V2+" />

# Epic: Tay Copilot V2+ — Multi-turn, Streaming & Infrastructure

**Status:** Not started
**Priority:** Future (after V1 ships)
**Depends on:** [Tay Copilot V1](?path=/docs/product-epics-tay-copilot-v1--docs)
**See also:** [Schema Enrichment](?path=/docs/product-epics-schema-org-type-enrichment--docs) (richer item types for Tay suggestions)
...
This epic collects everything deferred from V1 that makes Tay a production-grade conversational assistant.
...
V1 is stateless — each message is independent. V2 introduces server-side conversation history so Tay can reference prior messages, follow up on suggestions, and maintain coherent multi-turn dialogue.
...
Tay V1 cannot see what the user already has. V2 injects relevant items (inbox, projects, next actions) into the system prompt so Tay can make context-aware suggestions — e.g. linking a new action to an existing project.
...
Add GitLab CI stages that validate agent-related code.
