...
Every successful deploy to production pushes SHA-tagged container images to Harbor and auto-commits the new tag to the Kubernetes kustomization manifest. However there are no named releases, no git tags, no release notes, and no archived testing evidence. Additionally, all test jobs have `allow_failure: true`, meaning broken code can reach production without any gate.
...
3. **Re-tags container images** in Harbor using `crane tag` — adds a `v0.x.y` alias to the existing SHA-tagged manifest without rebuilding or transferring layers
...
8. **Creates GitLab release** via `glab release create` with the generated notes
9. Prints a reminder to push — does NOT push (per project conventions)

**Usage:** `./ci/release.sh 0.2.0`

**Prerequisites:** `brew install crane`, `glab` authenticated, Harbor credentials available to crane.
...
# Install crane
brew install crane
crane auth login harbor.100-114-108-51.nip.io:30443 -u $HARBOR_USER -p $HARBOR_PASSWORD --insecure
...
crane ls harbor.100-114-108-51.nip.io:30443/terminandoyo/frontend --insecure  # shows both SHA and v0.1.0
glab release view v0.1.0             # shows release with notes
...
Remove `allow_failure: true` from all 11 quality/test jobs in `.gitlab-ci.yml`. Add `needs:` to build jobs so they depend on quality+test completion:

```yaml
build-frontend:
  needs: [frontend-lint, frontend-typecheck, frontend-unit, frontend-storybook, frontend-integration]
...
Add `--reporter=junit --outputFile=test-results/junit.xml` to vitest jobs and `--junitxml=test-results/junit.xml` to pytest jobs. Declare as `reports: junit:` artifacts for native GitLab test report integration.
...
push-frontend:
  ...
  script:
    - crane push /tmp/frontend.tar "${HARBOR_REGISTRY}/terminandoyo/frontend:${CI_COMMIT_SHORT_SHA}" --insecure
```
...
1. Download pipeline artifacts for the deployed SHA via `glab api`
2. Bundle test reports, coverage, and Playwright reports into `testing-evidence-v0.x.y.tar.gz`
3. Attach to the GitLab release via `glab release upload`
4. Include pipeline URL in release notes

### 2f. VERIFY

```bash
# Rebuild CI image with crane
glab ci run --variables BUILD_CI_IMAGE=true
...
./ci/release.sh 0.2.0
glab release view v0.2.0   # should show attached testing-evidence archive
```
...
smoke-test:
  ...
  script:
    - sleep 30
    - curl -fsSk "https://terminandoyo.100-114-108-51.nip.io:30443/" || exit 1
    - curl -fsSk "https://terminandoyo.100-114-108-51.nip.io:30443/api/health" || exit 1
```
...
- `.gitlab-ci.yml` — remove `allow_failure`, add `needs`, add JUnit artifacts, fix push stage
...
- [ ] `ci/release.sh 0.x.y` creates git tag, re-tags images, creates GitLab release
- [ ] `crane ls` shows both SHA and version tags for all 3 images
- [ ] `glab release view v0.x.y` shows release notes with commit log
- [ ] Quality/test failures block the build stage (Phase 2)
- [ ] JUnit reports appear in GitLab MR/pipeline UI (Phase 2)
