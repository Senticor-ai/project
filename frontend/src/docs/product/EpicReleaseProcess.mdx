...
Remove `allow_failure: true` from all 11 quality/test jobs in `.github/workflows/ci.yml`. Add explicit job dependencies so image publish jobs depend on quality+test completion:
...
- `.github/workflows/ci.yml` — remove `allow_failure`, add dependencies, add JUnit artifacts, fix push stage
