...
This document defines the **Detachable Attachment** pattern — the concrete implementation of Paperclip principle #5: *AI is a contributor, not an authority*.

---

## Core Principle

When Tay (or any AI agent) suggests creating, modifying, or organizing items, the suggestion is presented as a **preview card** in the conversation. The preview:
...
- **Avatar**: `chat_bubble` icon (same as Tay's chat avatar)
...
- Filtering items by creation method (manual vs. Tay)
...
Each tool call from Tay renders as one `TaySuggestionCard`. If Tay returns multiple tool calls in a single response, each gets its own card with an independent lifecycle.
...
They cannot add new items, change buckets, or modify other properties. For deeper changes, they should dismiss and ask Tay differently, or edit the items after creation.
