...
- Scopilots in chat history for context
...
```typescript
CaptureSource: { kind: "copilot", conversationId: "conv-{uuid}" }
```

The provenance history entry uses action `"created"` with a source string:

```typescript
{ action: "created", source: "copilot-suggestion", timestamp: "..." }
```
...
Each tool call from Copilot renders as one `CopilotSuggestionCard`. If Copilot returns multiple tool calls in a single response, each gets its own card with an independent lifecycle.
