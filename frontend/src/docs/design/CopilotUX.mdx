...
The chat lives in a **right side panel** — the first implementation of the
...
**Conversation list:** History toggle button in the panel header opens `ConversationList` — resume, archive, or start a new conversation.
...
During streaming, Copilot text messages render incrementally with an `isStreaming` flag — partial
