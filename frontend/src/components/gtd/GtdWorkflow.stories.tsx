...
import type { Thing, ItemEditableFields } from "@/model/gtd-types";
...
export const TriageToNext: Story = {
  render: () => (
    <WorkflowApp
      initialThings={[createThing({ title: "Review annual budget report" })]}
    />
  ),
  play: async ({ canvas, userEvent, step }) => {
...
export const TriageToWaiting: Story = {
  render: () => (
    <WorkflowApp
      initialThings={[createThing({ title: "Call client about Q1 proposal" })]}
    />
  ),
  play: async ({ canvas, userEvent, step }) => {
