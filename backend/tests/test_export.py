...
def test_export_json_returns_array_and_headers(auth_client):
    ...
    response = auth_client.get("/things/export")
...
def test_export_default_excludes_completed(auth_client):
    ...
    # Create active item
    _create_thing(auth_client, _action_thing(name="Active task"))

    # Create completed item (has endTime)
    _create_thing(
        auth_client,
        _action_thing(name="Done task", end_time="2025-06-15T10:00:00Z"),
    )

    response = auth_client.get("/things/export")
    assert response.status_code == 200
    payload = response.json()
    names = [item["thing"]["name"] for item in payload]
...
def test_export_default_excludes_archived(auth_client):
    ...
    archived = _create_thing(auth_client, _action_thing(name="Archived task"))
    auth_client.delete(f"/things/{archived['thing_id']}")

    response = auth_client.get("/things/export")
    assert response.status_code == 200
    names = [item["thing"]["name"] for item in response.json()]
    assert "Active task" in names
...
def test_export_include_archived(auth_client):
    ...
    _create_thing(auth_client, _action_thing(name="Active"))
    archived = _create_thing(auth_client, _action_thing(name="To archive"))
    auth_client.delete(f"/things/{archived['thing_id']}")

    response = auth_client.get("/things/export?include_archived=true")
    assert response.status_code == 200
    names = [item["thing"]["name"] for item in response.json()]
...
def test_export_include_completed(auth_client):
    ...
    response = auth_client.get("/things/export?include_completed=true")
    assert response.status_code == 200
    names = [item["thing"]["name"] for item in response.json()]
    assert "Active task" in names
...
def test_export_both_filters(auth_client):
    ...
    _create_thing(
        auth_client,
        _action_thing(name="Completed", end_time="2025-06-15T10:00:00Z"),
    )
    archived = _create_thing(auth_client, _action_thing(name="Archived"))
    auth_client.delete(f"/things/{archived['thing_id']}")

    response = auth_client.get(
        "/things/export?include_archived=true&include_completed=true"
    )
    assert response.status_code == 200
    names = [item["thing"]["name"] for item in response.json()]
    assert "Active" in names
